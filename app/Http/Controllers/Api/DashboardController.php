...
class DashboardController extends Controller
{
    ...
    public function setFiscalYear(Request $request){
        $fiscal_year = MstFiscalYear::pluck('id','code');
        ...
        if(isset($params) && count($params) > 0){
            ...
            $fiscal_year_id = AppSetting::where('client_id',1000)->pluck('fiscal_year_id')->first();
        }

        if($fiscal_year_status){
            $this->fiscal_year_id = $fiscal_year_id;
        }else{
            $this->fiscal_year_id = null;
        }
        $data['fiscal_year'] = $fiscal_year;
        $data['selected_fiscal_year_id'] = $fiscal_year_id;
