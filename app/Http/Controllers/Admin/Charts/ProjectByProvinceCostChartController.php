...
/**
 * Class ProjectByCostChartController
 * @package App\Http\Controllers\Admin\Charts
 * @property-read \Backpack\CRUD\app\Library\CrudPanel\CrudPanel $crud
 */
class ProjectByProvinceCostChartController extends ChartController
{
    protected $new_datas;
    ...
    public function setup()
    {
        ...
        // $this->chart->labels(['Province']);
        $this->chart->labels(['States']);

        // RECOMMENDED. Set URL that the ChartJS library should call, to get its data using AJAX.
        $this->chart->load(backpack_url('charts/project-by-province-cost'));
    ...
    public function response()
    {
        ...
        foreach($response as $key => $rd){
            ...
            $response[$key]->chart_name= 'project_by_cost';
        }
    ...
    public function data()
    {
        ...
        $projects_cost = DB::table('pt_project as pp')->select('mfp.name_lc','mfp.id as province_id',DB::raw('sum(pp.project_cost) as project_cost'))
        ...
        foreach($datas as $rows){
            foreach($rows as $row){
                ...
                $new_datas[$row->province_id]['data'][] = intval($row->project_cost);
