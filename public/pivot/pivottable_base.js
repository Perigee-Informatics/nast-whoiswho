window.pivotTableHelper = (function() {
    ...
    var preparePivotOptions = function(opts) {
        ...
        //define variable & load the setting provided in options.
        // var pivotOpt = {
        //     rows: opts["rows"] !== undefined ? JSON.parse(JSON.stringify(opts["rows"])) : [],
        //     cols: opts["cols"] !== undefined ? JSON.parse(JSON.stringify(opts["cols"])) : [],
        //     renderers: opts["renderers"] !== undefined ? JSON.parse(JSON.stringify(opts["renderers"])) : [],
        //     aggregators: opts["aggregators"] !== undefined ? JSON.parse(JSON.stringify(opts["aggregators"])) : [],
        //     hiddenAttributes: opts["hiddenAttributes"] !== undefined ? JSON.parse(JSON.stringify(opts["hiddenAttributes"])) : [],
        //     inclusions: opts["inclusions"] !== undefined ? JSON.parse(JSON.stringify(opts["inclusions"])) : [],
        //     sorters: opts["sorters"] !== undefined ? JSON.parse(JSON.stringify(opts["sorters"])) : [],
        //     derivedAttributes: opts["derivedAttributes"] !== undefined ? JSON.parse(JSON.stringify(opts["derivedAttributes"])) : {},
        // };
        var pivotOpt = {
        ...
        // setting default derivedAttributes
        // pivotOpt.derivedAttributes["Gender."] = function (record) {
        //     return record.gender == 'F' ? "Female" : "Male";
        // };
        // pivotOpt.derivedAttributes["उमेर समुह"] = function(record) {
        //         if (record.age == null)
        //             return "NOT SET";
        //         else if (record.age < 25)
        //             return "Below 25 years";
        //         else if (record.age >= 65)
        //             return "Above 65 years";
        //         else
        //             return (record.age - (record.age % 5)) + "_" + ((record.age - (record.age % 5)) + 4);
        //     }
        // pivotOpt.hiddenAttributes.push("gender");
        // pivotOpt.hiddenAttributes.push("age");
        ...
        pivotOpt.onRefresh = function(config) {
            ...
            if (incArr.length > 0)
                incArr.splice(0, 0, "<b style='font-size:16px;'> Criteria(s)</b>");
